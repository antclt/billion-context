...
| `BILI_HOST_USAGE_CREDIT` | Host-usage accounting mode (file-config key: `hostUsageCredit`). `postfold` (default since #660) = the usage reported to the host is the actually-forwarded (folded) request — the tokens the model really receives — matching `[acp-usage] input=`; sane for every host and free of the >100% drift the baseline caused on pi/omp/codex/ZCode. `baseline` = arm the `#408` uncompressed-baseline backfill, so the host sees the unfolded total; opt in only where a host's own accounting genuinely consumes that number. Pre-#660 values are still accepted: `auto` → `baseline`, `off` → `postfold`. |
