...
预压缩(preflight compress)发生在代理发出任何响应字节**之前**,大会话要跑好几分钟(摘要调用往返同一个上游)。这会超过客户端的响应头超时(undici 默认 300s):客户端中断,代理丢弃进行中的压缩、永不转发 —— 每几分钟一次的死循环。

所以当预压缩超过宽限期(默认 **30s**,可用 `BILI_PREFLIGHT_HOLD_MS` 覆盖)时,代理会提前提交响应:流式请求得到 `200` + `text/event-stream` + SSE 注释保活(`: bili-preflight`,每 15s 一次);非流式请求得到 `200` + `application/json` + 空白填充。提前提交带 `x-bili-preflight: compressing` 头。状态一旦提交,迟到的失败就无法再改写它,改为**带内**投递:流式用协议原生错误事件(`preflight_compress_failed`),JSON 在已发的 200 下返回同样的结构化错误体。快路径(预压缩在宽限期内完成)行为不变,保留完整的状态码语义。
